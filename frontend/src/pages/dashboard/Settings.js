...
const Settings = () => {
  ...
  // Handlers (replace with real backend integration)
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      <h2 className="text-2xl font-bold mb-6"><span className="text-orange-500">Account</span> <span className="text-purple-600">Settings</span></h2>
      <div className="mb-6 flex gap-2 flex-wrap">
        {TABS.map(t => (
          <button
            key={t.value}
            onClick={() => setTab(t.value)}
            className={`px-4 py-2 rounded-full font-medium border transition shadow-sm ${tab === t.value ? 'bg-gradient-to-r from-orange-500 to-purple-600 text-white border-orange-400' : 'bg-white text-purple-600 border-purple-200 hover:bg-orange-50'}`}
          >
            {t.label}
          </button>
        ))}
      ...
      <div className="bg-white rounded-xl shadow-md p-6 border-l-4" style={{ borderImage: 'linear-gradient(to bottom, #FF6A00, #7F00FF) 1' }}>
        {tab === 'profile' && (
          <form onSubmit={handleProfileSave} className="space-y-6">
            ...
            <button type="submit" className="bg-gradient-to-r from-orange-500 to-purple-600 text-white px-6 py-2 rounded font-bold hover:from-orange-600 hover:to-purple-700 transition">Save Changes</button>
        ...
        {tab === 'password' && (
          <form onSubmit={handlePasswordChange} className="space-y-6">
            ...
            <button type="submit" className="bg-gradient-to-r from-orange-500 to-purple-600 text-white px-6 py-2 rounded font-bold hover:from-orange-600 hover:to-purple-700 transition">Change Password</button>
        ...
        {tab === 'preferences' && (
          <form onSubmit={handlePrefSave} className="space-y-6">
            ...
            <button type="submit" className="bg-gradient-to-r from-orange-500 to-purple-600 text-white px-6 py-2 rounded font-bold hover:from-orange-600 hover:to-purple-700 transition">Save Preferences</button>
        ...
        {tab === 'actions' && (
          <div className="space-y-6">
            ...
            <button
              className="bg-gradient-to-r from-orange-500 to-purple-600 text-white px-6 py-2 rounded font-bold hover:from-orange-600 hover:to-purple-700 transition"
